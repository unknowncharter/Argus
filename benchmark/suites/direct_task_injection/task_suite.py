...
_DATA_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "suites" / "direct_task_injection"


class DirectInjectionEnvironment(TaskEnvironment):
...
def fetch_resource(url: str) -> str:
    ...
    :param url: Full URL (e.g. mock server ``/web/...``).
    """
    try:
...
task_suite = TaskSuite[DirectInjectionEnvironment](
    "direct_task_injection",
    DirectInjectionEnvironment,
    TOOLS,
    _DATA_PATH,
)
