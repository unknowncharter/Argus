...
_DATA_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "suites" / "command_execution_injection"


class CommandExecEnvironment(TaskEnvironment):
...
def fetch_resource(url: str) -> str:
    ...
    :param url: Full URL (e.g. mock server ``/web/...`` documentation).
    """
    try:
...
task_suite = TaskSuite[CommandExecEnvironment](
    "command_execution_injection",
    CommandExecEnvironment,
    TOOLS,
    _DATA_PATH,
)
