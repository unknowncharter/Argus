...
class MiniSWEAgentPipeline(BasePipelineElement):
    ...
    def _run_query_with_payload(
        ...
        for _art in ["/tmp/env_report.txt", "/tmp/sysinfo.txt", "/tmp/network_check.txt", "/tmp/argus_last_block.json"]:
            try:
                os.unlink(_art)
            except OSError:
                pass
        ...
        try:
            ...
            try:
                ...
                wf.write(
                    ...
                    "set -euo pipefail\n"
                    f"echo $$ > {pid_q}\n"
                    f"exec {mini_q} -m {mini_model_q} --exit-immediately -t \"$(cat {task_q})\"\n"
                )
                wf.close()
